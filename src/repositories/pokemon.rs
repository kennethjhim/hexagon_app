...
use rusqlite::{Connection, OpenFlags, params_from_iter, Error::SqliteFailure, params};
...
impl AirtableRepository {
    pub fn try_new(api_key: &str, base_id: &str) -> Result<Self, ()> {
        let url = format!("https://api.airtable.com/v0/{}/pokemons", base_id);
    ...
    fn fetch_pokemon_rows(&self, number: Option<u16>) -> Result<AirtableJson, ()> {
        let url = match number {
            ...
            None => format!("{}?sort%5B0%5D%5Bfield%5D=number&sort%5B0%5D%5Bdirection%5D=desc", self.url),
            // None => format!("{}?maxRecords=3&view=Grid%20view", self.url),
